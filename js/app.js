(() => {
  ...
  function renderCategory(category) {
    ...
    const legend = $('layer-legend'); legend.hidden = category === 'roads';
    if (category === 'roads') { trafficLayer.addTo(map); legend.innerHTML = '<strong>地震・道路規制の凡例</strong><span><i class="legend-swatch traffic-swatch"></i>公式交通規制</span> <span><i class="legend-swatch volume-swatch"></i>交通量観測点</span> <span><i class="legend-swatch user-swatch"></i>利用者記録</span><br>交通量は参考情報です。規制データが未接続の場合は、公式サイトで確認してください。'; }
    else if (category === 'flood') { layers.flood.addTo(map); legend.innerHTML = '<strong>洪水・冠水の凡例</strong><span><i class="legend-swatch" style="background:#83b9e5"></i>洪水浸水想定区域（想定最大規模）</span><br>現在の浸水状況や通行可能性を示すものではありません。<br><strong>※この情報はリアルタイムではありません。</strong>最新の状況は公式情報をご確認ください。'; }
